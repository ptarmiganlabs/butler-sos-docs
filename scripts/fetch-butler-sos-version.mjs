/**
 * Fetch latest Butler SOS release tag and write to docs/.vitepress/version.js
...
async function writeVersionFile(version) {
  const content = `// Auto-generated by scripts/fetch-butler-sos-version.mjs – do not edit\nexport const version = '${version}';\n`;
...
async function main() {
  try {
    const headers = { "User-Agent": "butler-sos-docs-build-script" };
    ...
    // Normalize: keep from first 'v' onwards (e.g., 'butler-sos-v13.0.0' -> 'v13.0.0')
    const vIndex = rawTag.indexOf("v");
    const tag = vIndex >= 0 ? rawTag.slice(vIndex) : rawTag;

    await writeVersionFile(tag);
    console.log(`[butler-sos-docs] Latest ${REPO} version: ${tag}`);
  } catch (err) {
    console.warn(
      `[butler-sos-docs] Warning: Could not fetch latest release tag: ${err.message}`
    );
    // If the file already exists, keep it. Otherwise write a safe default.
    try {
      const existing = await fs.readFile(outFile, "utf8");
      if (existing) {
        console.log("[butler-sos-docs] Using existing generated version.js");
        return;
      }
    } catch {}
    const fallback = "v0.0.0";
    await writeVersionFile(fallback);
    console.log(`[butler-sos-docs] Wrote fallback version ${fallback}`);
  }
}
