...
The full field specification, including field types, maximum lengths, and example payloads, is available in the [UDP Payload Format Reference](/v14.0/reference/udp-payload-format#user-events-udp-payload).

## Message Queue

Butler SOS uses a managed queue to handle incoming user event UDP messages. The queue provides controlled concurrency, optional rate limiting, message size validation, and backpressure detection. Queue health metrics can optionally be stored in InfluxDB for monitoring. See the [UDP Message Queue](/v14.0/concepts/monitoring/udp-queue) documentation for details.
