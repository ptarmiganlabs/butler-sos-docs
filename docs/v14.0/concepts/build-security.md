...
For reference deployments with recommended security configurations, see the [Docker Compose examples](/v14.0/examples/docker-compose-stack).
...
When disabled, Butler SOS will not execute any OS commands for gathering host information. Note that [telemetry](/v14.0/concepts/features/telemetry) requires system information to be enabled.