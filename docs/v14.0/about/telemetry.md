...
Telemetry can be enabled/disabled in the Butler SOS configuration file using the `Butler-SOS.anonTelemetry` setting. See the [configuration file format reference](/v14.0/reference/config-file-format) for details.
...
More details about the systemInfo setting can be found in the [configuration file documentation](/v14.0/reference/config-file-format).
:::
...
The [legal page](/v14.0/legal-stuff#telemetry-data) has more information about this.
...
2. Run the created string through a [one-way hashing/message digest function](https://en.wikipedia.org/wiki/Cryptographic_hash_function).  
   Butler SOS uses Node.js' own [Crypto](https://nodejs.org/v14.0/latest-v15.x/api/crypto.html) library to create a [SHA-256](https://en.wikipedia.org/wiki/SHA-2) hash, using the default network interface's MAC address as salt.  
