...
A description of the config file format is available in the [Configuration File Format](/v14.0/reference/config-file-format) reference.
...
Update the config file as needed (see the [config file reference page](/v14.0/reference/config-file-format) for details).
...
If using Docker, the `NODE_ENV` environment variable is set in the `docker-compose.yml` file (as already done in the [template docker-compose files](https://github.com/ptarmiganlabs/butler-sos/tree/master/v14.0/docker-compose)).
