...
But how can you know whether Butler SOS itself is working? Somehow Butler SOS should be monitored.

Butler SOS (and most other tools in the Butler family) has a **heartbeat** feature. It sends periodic messages to a monitoring tool, which can then alert if Butler SOS hasn't checked in as expected.

[Healthchecks.io](https://healthchecks.io/) is an example of such a tool. It's open source and can be self-hosted, but also has a SaaS option if so preferred.
...
More info on using Healthchecks.io with Butler tools can be found [in this blog post](https://ptarmiganlabs.com/blog/2020/07/26/black-box-monitoring-of-butler-tools-monitoring-the-monitor/).
