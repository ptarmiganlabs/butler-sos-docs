...
Usually it's not that difficult, but if you're new to Docker and has no previous experience with monitoring tools, [using InfluxDB](/v14.0/getting-started/install/influxdb-grafana) is usually a bit easier.
...
Assuming you've already completed the [configuration of Butler SOS](/v14.0/reference/config-file-format), the result of running the `docker-compose_fullstack_prometheus.yml` file above is something like this:
