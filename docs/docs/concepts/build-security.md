...
For reference deployments with recommended security configurations, see the [Docker Compose examples](/docs/examples/docker-compose-stack).
...
When disabled, Butler SOS will not execute any OS commands for gathering host information. Note that [telemetry](/docs/concepts/features/telemetry) requires system information to be enabled.