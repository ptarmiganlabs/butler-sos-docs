...
# Getting rid of Sense log noise
...
Log4Net assigns a log level to all log events, these are one of the following: `DEBUG`, `INFO`, `WARN`, `ERROR` and `FATAL`.
...
Maybe the events occur at irregular intervals and therefore are more difficult to detect.
...
This means that when creating Grafana dashboards, it's possible to create a table that shows the number of "user-certificate" events and the number of "active-directory" events - and the number of "other" events.
...
<ResponsiveImage
  src="/img/concepts/log-events/categorising/butler-sos-grafana-log-event-categorise-1.png"
  alt="Butler SOS log events dashboard in Grafana. Uncategorized events by the yellow arrows, and in table/charts to the right."
  caption="Butler SOS log events dashboard in Grafana. Uncategorized events by the yellow arrows, and in table/charts to the right."
/>
