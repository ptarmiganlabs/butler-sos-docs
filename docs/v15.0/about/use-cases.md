...
A complete list of metrics is available in the [reference section](/v15.0/reference/).
...
Knowing how many users are logged at any given time gives a Sense admin an understanding of when peak hours are, when service windows should be planned, whether the server(s) is too small or too big etc.
...
For example, if the cache hit ratio goes down over weeks or months, that could mean a poorer user experience over that same time period.
...
## Log Events: Qlik Sense Errors & Warnings
...
This is a very powerful feature, but when enabled in Qlik Sense (from the QMC) it will generate _a lot_ of data that will all be stored in the Sense log files.
...
- The data is stored in a time series database, making it easy to visualize, analyze and alert on in real time.
...
When enabled via the QMC, the associated Sense log files can easily reach millions of lines per day even on a relatively small Sense environment.  
