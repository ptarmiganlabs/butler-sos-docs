...
If you want to quickly bring up a complete stack with Butler SOS, InfluxDB, and Grafana to see it in action, check out the [Docker Compose Stack Example](/v15.0/examples/docker-compose-stack).
:::
...
1. Copy [docker-compose.yml](https://github.com/ptarmiganlabs/butler-sos/blob/master/v15.0/docker-compose/docker-compose.yml) from the GitHub repository to the Butler SOS directory that was created above. The directory where the docker-compose file is stored is the 'root' directory of Butler SOS - everything else is relative to this directory.
