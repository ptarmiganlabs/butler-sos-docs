...
- **No entries in `allowedSources`**: A warning is logged and source validation is **automatically disabled** — all sources are accepted (fail-open behavior). Set `enableSourceValidation: true` _and_ provide at least one entry in `allowedSources` to restrict sources.
...
When `enableSourceValidation: true` and `allowedSources` is empty:

- Source validation is **automatically disabled** and a warning is logged.
- **All sources are accepted** (fail-open / allow-all).
- This matches the behavior of Butler (the companion tool) and avoids an accidental self-lockout when deploying the feature incrementally.

To effectively restrict sources, you must set `enableSourceValidation: true` _and_ provide at least one resolvable entry in `allowedSources`.

If all entries in `allowedSources` fail DNS resolution at startup, source validation is also automatically disabled with an error log.
